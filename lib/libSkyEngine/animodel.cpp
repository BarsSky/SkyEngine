...
glm::mat4 VulkanglTFModelAnimate::Node::getLocalMatrix() const {
    auto local_matrix = glm::translate(glm::mat4(1.0f), translation)
            * glm::toMat4(rotation)
...
*/
VulkanglTFModelAnimate::~VulkanglTFModelAnimate() {
...
void VulkanglTFModelAnimate::loadImages(tinygltf::Model &input) {
    ...
    for (size_t i = 0; i < input.images.size(); i++) {
        ...
        images[i].texture.fromBuffer(buffer, bufferSize, VK_FORMAT_R8G8B8A8_UNORM, glTFImage.width, glTFImage.height,
...
void VulkanglTFModelAnimate::loadTextures(tinygltf::Model &input) {
...
void VulkanglTFModelAnimate::loadMaterials(tinygltf::Model &input) {
...
VulkanglTFModelAnimate::Node *VulkanglTFModelAnimate::findNode(Node *parent, uint32_t index) {
...
VulkanglTFModelAnimate::Node *VulkanglTFModelAnimate::nodeFromIndex(uint32_t index) {
    Node *nodeFound = nullptr;
    for (auto &node: nodes) {
...
void VulkanglTFModelAnimate::loadSkins(tinygltf::Model &input) {
    ...
    for (size_t i = 0; i < input.skins.size(); i++) {
        ...
        if (glTFSkin.inverseBindMatrices > -1) {
            ...
            enma::Buffer * ptr_ssbo = &skins[i].ssbo;
            ...
            VK_CHECK_RESULT(vulkanDevice->createBuffer(
                    ...
                    ptr_ssbo,
                     (void*)(skins[i].inverseBindMatrices.data())));
...
void VulkanglTFModelAnimate::loadAnimations(tinygltf::Model &input) {
...
void VulkanglTFModelAnimate::loadNode(const tinygltf::Node &inputNode, const tinygltf::Model &input,
                                      VulkanglTFModelAnimate::Node *parent, uint32_t nodeIndex,
                                      std::vector<uint32_t> &indexBuffer,
                                      std::vector<VulkanglTFModelAnimate::Vertex> &vertexBuffer) {
    VulkanglTFModelAnimate::Node *node = new VulkanglTFModelAnimate::Node{};
    node->parent = parent;
...
glm::mat4 VulkanglTFModelAnimate::getNodeMatrix(VulkanglTFModelAnimate::Node *node) {
    glm::mat4 nodeMatrix = node->getLocalMatrix();
    VulkanglTFModelAnimate::Node *currentParent = node->parent;
    while (currentParent) {
...
void VulkanglTFModelAnimate::updateJoints(VulkanglTFModelAnimate::Node *node) {
    if (node->skin > -1) {
...
void VulkanglTFModelAnimate::updateAnimation(float deltaTime) {
...
void VulkanglTFModelAnimate::drawNode(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout,
                                      VulkanglTFModelAnimate::Node node) {
    if (node.mesh.primitives.size() > 0) {
        ...
        for (VulkanglTFModelAnimate::Primitive &primitive: node.mesh.primitives) {
            if (primitive.indexCount > 0) {
                // Get the texture index for this primitive
                VulkanglTFModelAnimate::Texture texture = textures[materials[primitive.materialIndex].baseColorTextureIndex];
...
void VulkanglTFModelAnimate::draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout) {
