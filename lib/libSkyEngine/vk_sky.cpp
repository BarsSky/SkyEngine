...
void VKSky::set_first_camera() {
  ...
  camera.setPerspective(
      45.0f, (float)*current_window.width / (float)*current_window.height, 0.1f,
      10000000.0f);
...
void VKSky::visible_ui(bool flag) const { u_ptr_->visible_ui(flag); }

VKAPI_ATTR VkBool32 VKAPI_CALL
VKSky::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
...
void VKSky::drawFrame() {
  ...
  auto tDiff =
      std::chrono::duration<double, std::milli>(tStart - lastTimestamp).count();
...
void VKSky::recreateCommandBuffer() { u_ptr_->viewUpdated = true; }

Object *VKSky::createObject(const pipelineObject &pipeline,
...
#ifdef QT_LIB_ENABLE
void VKSky::magickKeyboard(
    QKeyEvent *event, int key,
    int type) // int key, int scancode, int action, int mods);
{
...
void VKSky::magickScroll(QWheelEvent *event, double y_offset) {
  // if (camera.type == ObjCamera::lookat || camera.type ==
  // ObjCamera::lookat_v2)
  // {
...
void VKSky::magickMouse(
    QMouseEvent *event) // int button, int action, int mods);
{
  switch (event->button()) {
  case Qt::LeftButton:
...
void VKSky::magickCursor(QMouseEvent *event) // x_pos, double y_pos);
{
  ...
  //    mouseMoved(static_cast<float>(x_pos), static_cast<float>(y_pos),
  //    handled);

  if (handled) {
    mousePosition = glm::vec2(static_cast<float>(event->pos().x()),
                              static_cast<float>(event->pos().y()));
    return;
  }
  if (camera.type == ObjCamera::WORLD) {
  ...
  while (!magick_keyboard.core.all_done()) {
    ...
    if (_state != nullptr) {
      ...
      case MOUSE:
        ...
        magickMouse(_mouse->mouse_button, _mouse->mouse_action,
                    _mouse->mouse_mods);
        break;
      case CURSOR:
...
#ifdef QT_LIB_ENABLE
  VkWidget *VKSky::get_vk_widget(QWidget * parent) {
    vulkan_widget = u_ptr_->get_vk_widget(parent);
    return vulkan_widget;
  }
#endif

  void VKSky::quit_from_engine(vk_sky::VKSky * obj) { obj->done = true; }
...
#ifdef QT_LIB_ENABLE
  void VKSky::send_key_event(void *ev, int key, int type) {
    magickKeyboard(reinterpret_cast<QKeyEvent *>(ev), key, type);
  }

  void VKSky::send_mouse_event(void *event) {
    magickMouse(reinterpret_cast<QMouseEvent *>(event));
  }

  void VKSky::send_cursor_event(void *event) {
    magickCursor(reinterpret_cast<QMouseEvent *>(event));
  }

  void VKSky::send_scroll_event(void *event, double y_offset) {
...
#ifdef QT_LIB_ENABLE
    magick_thread = std::thread(&VKSky::mainLoop, this);
#else
  mainLoop();
#endif
  }

  Screen vk_sky::VKSky::getScreen() { return u_ptr_->getScreen(); }

  float vk_sky::VKSky::get_timer() { return u_ptr_->frameTimer; }

  void vk_sky::VKSky::set_recreate_event(bool flag) { is_recreate = flag; }

  void vk_sky::VKSky::recreateWindow() { u_ptr_->recreateSwapChain(); }

  void VKSky::removeObject(Object * _obj) { u_ptr_->RemoveObject(_obj); }

// #endif
#ifdef QT_LIB_ENABLE

#include "moc_vk_sky.cpp"
