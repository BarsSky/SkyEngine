...
#ifdef QT_LIB_ENABLE
void VKSky::magickKeyboard(QKeyEvent *event, int key, int type) // int key, int scancode, int action, int mods);
{
...
{
    // if (camera.type == ObjCamera::lookat || camera.type == ObjCamera::lookat_v2)
    // {
...
void VKSky::magickMouse(QMouseEvent *event) // int button, int action, int mods);
{
    switch (event->button())
    {
    case Qt::LeftButton:
