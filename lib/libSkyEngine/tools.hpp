...
constexpr void VK_CHECK_RESULT(VkResult res, const char *file = __FILE__,
                               int line = __LINE__) {
  if (res != VK_SUCCESS) {
...
template <class T> struct optional {
  ...
  auto operator!=(optional elem) -> bool { return _value != elem.value(); };
...
struct QueueFamilyIndices {
  ...
  tools::optional<uint32_t> presentFamily;
...
static void
_DestroyDebugUtilsMessengerEXT(VkInstance instance,
...
inline std::vector<std::string> split(std::string stringToBeSplitted,
...
inline VkBool32 getSupportedDepthFormat(VkPhysicalDevice physicalDevice,
...
inline std::vector<VkAttachmentReference> set_color_attachments_reference() {
  ...
  for (size_t attach = 0; attach < color_reference_vector.size(); attach++) {
    ...
    color_reference_vector.at(attach).layout =
...
inline VkPipelineShaderStageCreateInfo LoadShader(VkDevice _device,
