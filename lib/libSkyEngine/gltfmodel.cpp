...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, VulkanDevice *device,
  ...
  if (!isKtx) {
    ...
    device->endSingleTimeCommands(copyCmd, copyQueue, true);

    vkFreeMemory(device->logicalDevice, stagingMemory, nullptr);
    vkDestroyBuffer(device->logicalDevice, stagingBuffer, nullptr);

    // Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
    VkCommandBuffer blitCmd = device->beginSingleTimeCommands(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    for (uint32_t i = 1; i < mipLevels; i++) {
      ...
      mipSubRange.layerCount = 1; {
        ...
        vkCmdPipelineBarrier(blitCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0,
                             nullptr, 1, &imageMemoryBarrier);
      }

      vkCmdBlitImage(blitCmd, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    device->endSingleTimeCommands(blitCmd, copyQueue, true);
  } else {
    ...
    tools::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          subresourceRange);
    vkCmdCopyBufferToImage(copyCmd, stagingBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           static_cast<uint32_t>(bufferCopyRegions.size()), bufferCopyRegions.data());
    tools::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void vkglTF::Node::update() {
  if (mesh) {
    ...
    if (skin) {
      ...
      // mesh->uniformBase.matrix = m;
      memcpy(mesh->uniformBuffer.mapped, &mesh->uniformBase, sizeof(mesh->uniformBase));
    }
  }
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue) {
  ...
  VkCommandBuffer copyCmd = device->beginSingleTimeCommands(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
  tools::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                        subresourceRange);
  vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                         &bufferCopyRegion);
  tools::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
...
void vkglTF::Model::add_some_ubo() {
  increase_ubo = 1;
}
...
void vkglTF::Model::loadFromFile(std::string filename, VulkanDevice *device, VkQueue transferQueue,
  ...
  if ((fileLoadingFlags & FileLoadingFlags::PreTransformVertices) || (
        fileLoadingFlags & FileLoadingFlags::PreMultiplyVertexColors) || (fileLoadingFlags & FileLoadingFlags::FlipY)) {
  ...
  device->endSingleTimeCommands(copyCmd, transferQueue, true);
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node *node, VkDescriptorSetLayout descriptorSetLayout) {
  if (node->mesh) {
    ...
    descriptorSetAllocInfo.descriptorSetCount = 1;
    VK_CHECK_RESULT(
      vkAllocateDescriptorSets(device->logicalDevice, &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet
      ));
