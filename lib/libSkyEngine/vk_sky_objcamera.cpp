...
//  u_ptr_camera->camera.minPitch = -M_PI/2;
//  u_ptr_camera->camera.maxPitch = M_PI/2;
...
bool ObjCamera::updatePad(glm::vec2 axisLeft, glm::vec2 axisRight, float deltaTime) {
  bool retVal = false;
...
glm::mat4 ObjCamera::getRotationMatrix(glm::vec3 point_to, glm::vec3 point_from) {
...
glm::vec3 ObjCamera::getRotationVector(glm::vec3 point_to, glm::vec3 point_from) {
  glm::vec3 lookVector = point_to;
  if (lookVector == point_from)
    return glm::vec3(0);
  glm::vec3 direction = glm::normalize(lookVector - point_from);
...
//        auto quat = glm::quatLookAt(glm::normalize(rotation), glm::vec3(1, 0, 0));
  auto translate_matrix = glm::translate(glm::mat4(1.0f), position);
//        auto rotate_matrix = getRotationMatrix(glm::vec3(glm::radians(rotation)), glm::vec3(0));
...
glm::vec3 ObjCamera::getUp() const {
  return -glm::transpose(matrices.view)[1];
}
...
void ObjCamera::changeHeading(float degree) {
  ...
  if ((cam_pitch > 90 && cam_pitch < 270) || (cam_pitch < -90 && cam_pitch > -270))
    cam_head -= degree;
  else
    cam_head += degree;
...
glm::quat ObjCamera::safeQuatLookAt(const glm::vec3 &position, const glm::vec3 &target, const glm::vec3 &up,
...
ObjCamera::ObjCamera() {
  u_ptr_camera = std::make_unique<Camera_CImpl>();
}
...
void ObjCamera::setPerspective(float fov, float aspect, float znear, float zfar) {
  this->fov = fov;
  this->znear = znear;
  this->zfar = zfar;
...
float ObjCamera::getNearClip() {
  return znear;
}

float ObjCamera::getFarClip() {
  return zfar;
}
