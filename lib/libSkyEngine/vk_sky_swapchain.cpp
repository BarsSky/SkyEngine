...
void VulkanSwapChain::createAttacment(VkFormat format, VkImageUsageFlags usage, FrameBufferAttachments *attachment) {
    ...
    VK_CHECK_RESULT(vkCreateImage(vDevice->logicalDevice, &image, nullptr, &attachment->image));
    vkGetImageMemoryRequirements(vDevice->logicalDevice, attachment->image, &memReqs);
    memAlloc.allocationSize = memReqs.size;
    memAlloc.memoryTypeIndex = vDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    VK_CHECK_RESULT(vkAllocateMemory(vDevice->logicalDevice, &memAlloc, nullptr, &attachment->mem));
    VK_CHECK_RESULT(vkBindImageMemory(vDevice->logicalDevice, attachment->image, attachment->mem, 0));
    ...
    VK_CHECK_RESULT(vkCreateImageView(vDevice->logicalDevice, &imageView, nullptr, &attachment->view));
}

void VulkanSwapChain::createGBufferAttachments() {
    // ! Не забывать отслеживать формат картинки для отображения
    createAttacment(VK_FORMAT_R16G16B16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &attachments.position);
    createAttacment(VK_FORMAT_R16G16B16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &attachments.normal);
    createAttacment(VK_FORMAT_R8G8B8_UNORM, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &attachments.position);
}

void VulkanSwapChain::create(int *width, int *height, bool vsync, bool fullscreen) {
    // Store the current swap chain handle so we can use it later on to ease up recreation
    VkSwapchainKHR oldSwapchain = swapChain;

    // Get physical device surface properties and formats
    VkSurfaceCapabilitiesKHR surfCaps;
    VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vDevice->physicalDevice, vDevice->surface, &surfCaps));

    // Get available present modes
    uint32_t presentModeCount;
    VK_CHECK_RESULT(
            vkGetPhysicalDeviceSurfacePresentModesKHR(vDevice->physicalDevice, vDevice->surface, &presentModeCount,
                                                      NULL));
    ...
    VK_CHECK_RESULT(vkCreateSwapchainKHR(vDevice->logicalDevice, &swapchainCI, nullptr, &swapChain));

    // If an existing swap chain is re-created, destroy the old swap chain
    // This also cleans up all the presentable images
    if (oldSwapchain != VK_NULL_HANDLE) {
        for (uint32_t i = 0; i < imageCount; i++) {
            vkDestroyImageView(vDevice->logicalDevice, swapChainImageViews[i], nullptr);
        }
        vkDestroySwapchainKHR(vDevice->logicalDevice, oldSwapchain, nullptr);
    }
    VK_CHECK_RESULT(vkGetSwapchainImagesKHR(vDevice->logicalDevice, swapChain, &imageCount, NULL));

    // Get the swap chain images
    swapChainImages.resize(imageCount);
    VK_CHECK_RESULT(vkGetSwapchainImagesKHR(vDevice->logicalDevice, swapChain, &imageCount, swapChainImages.data()));
...
void VulkanSwapChain::cleanupSwapChain() {
    ...
    for (size_t i = 0; i < swapChainFramebuffers.size(); i++) {
        vkDestroyFramebuffer(vDevice->logicalDevice, swapChainFramebuffers.at(i), nullptr);
    }

    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        vkDestroyImageView(vDevice->logicalDevice, swapChainImageViews.at(i), nullptr);
    }
...
void VulkanSwapChain::createFramebuffersSW() {
    ...
    for (int i = 0; i < swapChainImageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(vDevice->logicalDevice, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) !=
            VK_SUCCESS) {
...
void VulkanSwapChain::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        swapChainImageViews[i] = createImageView(vDevice->logicalDevice, swapChainImages[i], swapChainImageFormat,
...
void VulkanSwapChain::createRenderPass() {
    ...
    // dependencies.back().srcSubpass = VK_SUBPASS_EXTERNAL;
    // dependencies.back().dstSubpass = 0;
    // dependencies.back().srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
...
void VulkanSwapChain::createColorResources() {
    ...
    createImage(swapChainExtent.width, swapChainExtent.height, 1, vDevice->msaaSamples, colorFormat,
                VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory);
    colorImageView = createImageView(vDevice->logicalDevice, colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
}

void VulkanSwapChain::createDepthResources() {
    VkFormat depthFormat = vDevice->findDepthFormat();

    createImage(swapChainExtent.width, swapChainExtent.height, 1, vDevice->msaaSamples, depthFormat,
                VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
    depthImageView = createImageView(vDevice->logicalDevice, depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT,
                                     mipLevels);
}

void
VulkanSwapChain::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout,
                                       uint32_t mipLevels) {
...
void VulkanSwapChain::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples,
    ...
    if (vkCreateImage(vDevice->logicalDevice, &imageInfo, nullptr, &image) != VK_SUCCESS) {
    ...
    allocInfo.memoryTypeIndex = vDevice->findMemoryType(memRequirements.memoryTypeBits, properties);

    if (vkAllocateMemory(vDevice->logicalDevice, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
...
void VulkanSwapChain::setRenderType(RenderType flag) {
    render_type = flag;
}
...
void VulkanSwapChain::searchSurfaceFormat() {
    ...
    uint32_t queueCount;
    vkGetPhysicalDeviceQueueFamilyProperties(vDevice->physicalDevice, &queueCount, NULL);
    assert(queueCount >= 1);

    std::vector<VkQueueFamilyProperties> queueProps(queueCount);
    vkGetPhysicalDeviceQueueFamilyProperties(vDevice->physicalDevice, &queueCount, queueProps.data());
    ...
    for (uint32_t i = 0; i < queueCount; i++) {
        vkGetPhysicalDeviceSurfaceSupportKHR(vDevice->physicalDevice, i, vDevice->surface, &supportsPresent[i]);
    }
    ...
    SwapChainSupportDetails swapChainSupport = vDevice->querySwapChainSupport(vDevice->getPhysicalDevice());
